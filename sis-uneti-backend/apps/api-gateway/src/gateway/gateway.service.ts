 ...
 *
 * ✅ Fix: Implementa OnModuleInit para que la suscripción a Redis
 * se haga de forma async correcta (no en el constructor).
 */
@Injectable()
export class GatewayService implements OnModuleInit {
  private readonly logger = new Logger(GatewayService.name);
  private readonly TIMEOUT_MS = 30000; // 30 segundos de timeout
  private respuestasPendientes: Map<
    string,
  ...
  /**
   * Se ejecuta automáticamente por NestJS cuando el módulo está listo.
   * ✅ Fix: La suscripción a Redis debe hacerse aquí (async/await correcto),
   * NO en el constructor (que no puede ser async).
   */
  async onModuleInit() {
    await this.redisService.subscribe('gateway.respuestas', (message) => {
    ...
    this.logger.log('📡 Gateway suscrito al canal: gateway.respuestas');
  }
      ...
      const timeout = setTimeout(() => {
        ...
        reject(
          new BadRequestException(
            `Timeout: El microservicio no respondió en ${this.TIMEOUT_MS}ms`,
          ),
        );
      }, this.TIMEOUT_MS);
